...
0.2 Implement the ENTIRE visual spec from `SEMA-Login-Spec.pdf` v1.3 §2: logo, headline — the English phrase "AI Business Advisor" in BOTH languages (brand tagline, never translated), email field + continue, divider, Google/Microsoft buttons. Copy convention (also in AGENTS.md): SEMA is feminine in all Hebrew copy. (official brand guidelines — icon, full label, equal size, no "primary" among them; rendered in disabled state with a "בקרוב"/"Coming soon" tooltip), "נתקלת בבעיה?" mailto, full RTL, bilingual copy (browser language), loading states, and the code step: 6-digit input (`autocomplete="one-time-code"`, autofocus), 60s resend countdown, "החלף אימייל" link.
...
0.6 **Sidebar footer placeholder (approved mockup + spec v1.4 §4) — visible in the product NOW with the mock identity:**
   - Restructure the sidebar footer into TWO rows: org row (existing workspace switcher + gear, unchanged) and BELOW it a user row — initials avatar, name, role, taken from the existing mock identity (`GET /api/admin/me`-equivalent): **Miri Levi · Admin** ("מירי לוי" in the Hebrew UI, avatar "ML"). The row must render the identity the server reports — not a hardcoded string — so it flips to the real user automatically when Part A lands.
   - Clicking the user row opens a small menu (`useDismiss`, keyboard accessible): "הגדרות אישיות" with a "בקרוב" tag (disabled), divider, "התנתקות" in danger color with logout icon.
   - Logout is a PLACEHOLDER wired to the mock flow: clicking it navigates to the `/login` page from 0.1-0.4 (a dev-mode "sign in" there returns to the app). No session is created or destroyed — this is the demonstrable loop until Part A replaces both ends.
   - Both org and user rows always present — never replace the org name with the user name. RTL-safe, bilingual.
